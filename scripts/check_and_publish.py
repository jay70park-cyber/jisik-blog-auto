...
def call_claude_revise(previous_draft, instruction, timeout=400, category="jisik"):
...
- 이미지는 정확히 2곳, 반드시 `[이미지: 한글설명 | search: 영어검색키워드]` 형식으로 쓸 것.
  `| search:` 뒤의 영어 키워드는 필수다. 이것이 없으면 사진이 삽입되지 않는다.
  영어 키워드는 지역명 없이 보편적인 장면으로 쓸 것.
  좋은 예: modern office interior / industrial building exterior / business meeting
  나쁜 예: dongtan technovalley (한국 지역명은 스톡사진에 없다)
