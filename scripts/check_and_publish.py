    ...
    if text.lower() in [w.lower() for w in APPROVE_WORDS]:
        ...
        if not os.path.exists(DRAFT_FILE):
            ...
            with open(DRAFT_FILE, "r", encoding="utf-8") as f:
                final_text = f.read()
            # <mark> 태그를 복사/붙여넣기해도 눈에 띄는 표시로 변환
            final_text = final_text.replace("<mark>", "【확인필요▶ ").replace("</mark>", " ◀확인필요】")

            final_path = os.path.join(STATE_DIR, "final_post.txt")
            with open(final_path, "w", encoding="utf-8") as f:
                f.write(final_text)

            send_telegram_document(
                final_path,
                "승인 확인했습니다! 네이버 블로그 글쓰기 API는 네이버 정책상 더 이상 제공되지 않아 "
                "자동 게시는 불가능합니다 (2020년 서비스 종료).\n"
                "아래 파일 내용을 복사해서 네이버 블로그 글쓰기 화면에 붙여넣고 발행해주세요.\n"
                "【확인필요▶ ... ◀확인필요】로 표시된 부분은 노란 형광펜으로 칠하고, 최신 수치인지 한 번 더 확인해주세요.",
            )
    else:
        print("수정 요청 감지: " + text)
        if not os.path.exists(DRAFT_FILE):
