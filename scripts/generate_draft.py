...
1. 제목: 핵심 인사이트가 드러나는, 독자(투자자/임차인)의 관심을 끄는 제목
   ※ 제목 바로 다음에는 "이번주는 {intro_cat}에 대해 알아보는 시간인데요 / {intro_cat} 관련 검색 키워드 분석결과 {intro_kw}가 1위로 확인되어..." 라는 소개 문장과, 키워드별 관심도 데이터 표가 시스템에 의해 자동으로 삽입됩니다. 당신은 이 소개 문장과 표를 직접 쓰지 마세요. 제목 다음 곧바로 2번(핵심 인사이트) 문단부터 작성하면 됩니다.
...
def render_images(markdown_text, rows):
    ...
    def _replace(match):
        ...
        if any(k in kor_part for k in ["스코어", "그래프", "차트", "비교", "지수"]) and rows:
            ...
            if chart:
                return ('<figure style="margin:20px 0;text-align:center;">'
                        '<img src="' + chart + '" style="max-width:60%;border-radius:8px;" alt="' + kor_part + '">'
        ...
        if photo:
            ...
            return ('<figure style="margin:20px 0;text-align:center;">'
                    '<img src="' + photo_url + '" style="max-width:60%;border-radius:8px;" alt="' + kor_part + '">'
        ...
        return ('<div style="margin:16px 0;padding:14px 16px;background:#f2f2f2;border:1px dashed #999;'
                'border-radius:6px;color:#666;font-size:14px;">📷 이미지 제안: ' + kor_part + '</div>')
...
def build_score_table_html(rows):
    ...
    trend_decimals = 2
    score_decimals = 2
    th_style = "border:1px solid #ddd;padding:6px 10px;background:#EEF2FB;color:#1F3C88;font-weight:700;font-size:13.5px;"
    th_style_right = th_style + "text-align:right;"
    td_style = "border:1px solid #ddd;padding:6px 10px;font-size:13.5px;"
    ...
    return (
        '<table style="border-collapse:collapse;width:100%;margin:16px 0;"><thead><tr>'
        '<th style="' + th_style + '">키워드</th>'
        '<th style="' + th_style_right + '">블로그 건수</th>'
        '<th style="' + th_style_right + '">카페 건수</th>'
        '<th style="' + th_style_right + '">검색트렌드지수</th>'
        '<th style="' + th_style_right + '">관심도스코어</th>'
...
_H1_STYLE = "font-size:22px;line-height:1.4;margin-top:30px;margin-bottom:12px;color:#1F3C88;font-weight:800;"
_H2_STYLE = "font-size:17px;line-height:1.3;margin-top:26px;margin-bottom:10px;color:#1F3C88;font-weight:800;border-bottom:2px solid #C9932F;padding-bottom:3px;"
_H3_STYLE = "font-size:16px;line-height:1.3;margin-top:20px;margin-bottom:8px;color:#1F3C88;font-weight:700;"
_P_STYLE = "line-height:1.8;margin:12px 0;color:#222;font-size:16px;"
_TABLE_STYLE = "border-collapse:collapse;width:100%;margin:16px 0;font-size:14.5px;"
_TH_BASE = "border:1px solid #ddd;padding:8px 10px;background:#EEF2FB;color:#1F3C88;font-weight:700;"
_TD_BASE = "border:1px solid #ddd;padding:8px 10px;"
_MARK_STYLE = "background:#fff176;padding:1px 3px;border-radius:2px;"
_BLOCKQUOTE_STYLE = "margin:6px 0 18px 0;padding:8px 14px;background:#f7f8fa;border-left:3px solid #C9932F;color:#555;font-size:13.5px;line-height:1.6;"
_STRONG_STYLE = "color:#B8451D;font-weight:700;"
_LI_STYLE = "line-height:1.8;margin:4px 0;"


def _inline_styles(html):
    """마크다운 변환 결과의 태그들에 인라인 style을 직접 삽입한다 (클래스 대신)."""
    html = re.sub(r"<h1>", '<h1 style="' + _H1_STYLE + '">', html)
    html = re.sub(r"<h2>", '<h2 style="' + _H2_STYLE + '">', html)
    html = re.sub(r"<h3>", '<h3 style="' + _H3_STYLE + '">', html)
    html = re.sub(r"<p>", '<p style="' + _P_STYLE + '">', html)
    html = re.sub(r"<table>", '<table style="' + _TABLE_STYLE + '">', html)
...
def render_naver_html(markdown_text, title="블로그 초안", category_display=None, top_keyword=None, rows=None):
    ...
    def _wrap_insight(match):
        inner = match.group(1)
        box_style = "background:#EEF2FB;border-radius:12px;padding:16px 18px;margin:20px 0 26px 0;color:#222;line-height:1.8;"
        label_style = "display:block;font-size:12px;font-weight:800;color:#C9932F;letter-spacing:1px;margin-bottom:6px;"
        return (
            '<div style="' + box_style + '">'
            '<span style="' + label_style + '">🗣️ 핵심 인사이트</span>'
            + inner +
            "</div>"
        )
    ...
    if category_display and top_keyword:
        ...
        intro_style = "background:#F5F6FA;border-radius:10px;padding:14px 16px;color:#444;font-size:15px;margin:14px 0 20px 0;line-height:1.7;"
        strong_style = "color:#1F3C88;font-weight:700;"
        intro_html = (
            '<p style="' + intro_style + '">이번주는 <strong style="' + strong_style + '">' + category_display + '</strong>에 대해 알아보는 시간인데요.<br>'
            + category_display + ' 관련 검색 키워드 분석결과 <strong style="' + strong_style + '">' + top_keyword + '</strong>가 1위로 확인되어, '
...
<body style="font-family:-apple-system,'Malgun Gothic',sans-serif;font-size:16px;line-height:1.8;color:#222;max-width:720px;margin:24px auto;padding:0 16px;">
{body_html}
</body>
</html>"""
    return html
