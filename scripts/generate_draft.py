...
6. **이미지 삽입 (실제 이미지로 자동 치환됨)**: 본문 중 정확히 2곳에 아래 형식으로 표시하세요. 반드시 독립된 문단으로(앞뒤에 빈 줄), 문장 중간에 끼워 넣지 마세요.
   - 첫 번째는 반드시 관심도 데이터 비교 그래프로: `[이미지: 관심도스코어 비교]` (이 표현이 있으면 시스템이 실제 막대그래프를 자동으로 그려 넣습니다)
   - 두 번째는 본문 주제와 어울리는 실사진으로: `[이미지: 한글로 어떤 장면인지 설명 | search: 검색용 영어 키워드 2~4단어]` 형식. 예: `[이미지: 지식산업센터 사무실 내부 | search: modern office interior]`. search 뒤 영어 키워드는 실제 스톡사진 검색에 쓰이므로, 구체적이고 일반적인 영단어로 작성하세요 (건물외관→office building exterior, 계약서→business contract signing, 도시전경→city skyline 등).
