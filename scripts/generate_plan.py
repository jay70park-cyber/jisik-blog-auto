...
def build_agenda_block(result):
    ...
    if notices:
        ...
        lines.append("    열람·의견청취·공람  → 심의 전")
        lines.append("    결정 고시·지형도면   → 확정")
        lines.append("    실시계획 인가        → 착공 직전")
        lines.append("    재결·수용·보상계획   → 보상 단계")
        lines.append("    준공·사용승인        → 완료")
        lines.append("  이 순서가 곧 사업의 진행 흐름입니다.")
        lines.append("")
        lines.append("  [고시를 다룰 때 반드시 지킬 것]")
        lines.append("  - 위 목록에 있는 것은 공고일과 제목뿐입니다.")
        lines.append("    입주 시점, 준공 예정일, 세대수, 면적, 사업비는 주어지지 않았습니다.")
        lines.append("    주어지지 않은 것을 추정해 쓰지 마세요.")
        lines.append("  - 승인일은 행정 절차가 진행된 날입니다. 입주 시기가 아닙니다.")
        lines.append("    승인일이 가깝다는 이유로 입주 시기가 맞물린다고 쓰지 마세요.")
        lines.append("  - 변경 승인의 차수(2차, 5차)는 조정이 여러 번 있었다는 뜻일 뿐입니다.")
        lines.append("    사업이 순조롭다거나 지연됐다는 뜻으로 읽지 마세요.")
        lines.append("  - 목록에 없는 단계는 '아직 확인되지 않았다'로 쓰세요.")
        lines.append("    예: 준공 고시가 없으면 '준공 고시는 아직 확인되지 않았다'.")
        lines.append("  - 고시 제목에는 위치·거리 정보가 없습니다.")
        lines.append("    어느 지구가 어디에 가깝다는 서술은 확인 전에는 쓰지 마세요.")
