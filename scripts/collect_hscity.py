...
def backfill(max_pages=150, stop_before=""):
    ...
    for board, url in BOARDS:
        ...
        for page in range(1, max_pages + 1):
            ...
            if not rows:
                print("  {}p 목록 없음 — 중단".format(page))
                break
            sig = tuple(r["고시번호"] for r in rows)
            if sig == seen_sig:
                print("  {}p 앞 페이지와 동일 — 중단".format(page))
                break
            seen_sig = sig
            scanned += len(rows)
            ...
            if stop_before and oldest and oldest < stop_before:
                print("  {}p 가 {} — 기준일 이전이라 중단".format(page, oldest))
                break
            ...
            if page % 25 == 0:
                print("  {}p 까지 — 훑음 {}건, 채택 {}건 (현재 {})".format(
                    page, scanned, kept_here, oldest))
            time.sleep(0.5)
        print("  {} 완료 — 훑음 {}건 중 {}건 채택\n".format(
            board, scanned, kept_here))

    print("누적 {}건 (신규 {}건)".format(len(merged), added))
    save_all(merged)
